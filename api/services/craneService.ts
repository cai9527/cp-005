...
class CraneService {
  ...
  validateUpdateInput(id: string, data: Partial<CraneCreateInput>): ValidationError[] {
    const errors: ValidationError[] = []
    const existing = craneRepository.findById(id)

    if (!existing) {
      errors.push({ field: 'id', message: '设备不存在' })
      return errors
    }
    ...
    if (data.name !== undefined && data.name !== null && String(data.name).trim() !== '') {
      ...
      if (name.length < 2) errors.push({ field: 'name', message: '设备名称至少2个字符' })
      if (name.length > 50) errors.push({ field: 'name', message: '设备名称不超过50个字符' })
      const duplicate = craneRepository.findByName(name)
      if (duplicate && duplicate.id !== id) {
        errors.push({ field: 'name', message: '设备名称已存在，请使用其他名称' })
      }
    }
    ...
    const validateNumber = (
      field: keyof Partial<CraneCreateInput>,
      label: string,
      min?: number,
      max?: number,
      allowZero = true
    ...
    const minR = data.min_radius !== undefined ? Number(data.min_radius) : existing.min_radius
    const maxR = data.max_radius !== undefined ? Number(data.max_radius) : existing.max_radius
    if (minR !== undefined && maxR !== undefined && !isNaN(minR) && !isNaN(maxR) && minR >= maxR) {
      errors.push({ field: 'min_radius', message: '最小工作幅度必须小于最大工作幅度' })
    }

    const minT = data.working_temp_min !== undefined ? Number(data.working_temp_min) : existing.working_temp_min
    const maxT = data.working_temp_max !== undefined ? Number(data.working_temp_max) : existing.working_temp_max
    if (minT !== undefined && maxT !== undefined && !isNaN(minT) && !isNaN(maxT) && minT >= maxT) {
      errors.push({ field: 'working_temp_min', message: '工作温度下限必须小于上限' })
    }

    const opWind = data.max_wind_operational !== undefined ? Number(data.max_wind_operational) : existing.max_wind_operational
    const nonOpWind = data.max_wind_nonoperational !== undefined ? Number(data.max_wind_nonoperational) : existing.max_wind_nonoperational
    if (opWind !== undefined && nonOpWind !== undefined && !isNaN(opWind) && !isNaN(nonOpWind) && opWind >= nonOpWind) {
      errors.push({ field: 'max_wind_operational', message: '工作风速必须小于非工作风速' })
    }

    const validateDate = (field: keyof Partial<CraneCreateInput>, label: string, allowFuture = true) => {
      const val = data[field]
      if (val === undefined || val === null || String(val).trim() === '') return
      const dateStr = String(val).trim()
      if (!DATE_REGEX.test(dateStr)) {
    ...
    const prodDate = data.production_date !== undefined ? String(data.production_date).trim() : existing.production_date
    const instDate = data.install_date !== undefined ? String(data.install_date).trim() : existing.install_date
    if (prodDate && instDate && DATE_REGEX.test(prodDate) && DATE_REGEX.test(instDate)) {
      if (new Date(prodDate) > new Date(instDate)) {
        errors.push({ field: 'install_date', message: '安装日期不能早于出厂日期' })
      }
    }
    ...
    if (data.serial_number !== undefined && data.serial_number !== null && String(data.serial_number).trim() !== '') {
      ...
      if (!SERIAL_NUMBER_REGEX.test(sn)) {
        ...
        const duplicate = craneRepository.findBySerialNumber(sn)
        if (duplicate && duplicate.id !== id) {
          errors.push({ field: 'serial_number', message: '该出厂编号已登记' })
        }
      }
    }
    ...
    if (data.registration_number !== undefined && data.registration_number !== null && String(data.registration_number).trim() !== '') {
      ...
      if (!REGISTRATION_REGEX.test(rn)) {
        ...
        const duplicate = craneRepository.findByRegistrationNumber(rn)
        if (duplicate && duplicate.id !== id) {
          errors.push({ field: 'registration_number', message: '该备案登记号已存在' })
        }
      }
    }

    const validateText = (field: keyof Partial<CraneCreateInput>, label: string, maxLen: number) => {
      const val = data[field]
      if (val === undefined || val === null || String(val).trim() === '') return
      if (String(val).length > maxLen) {
        errors.push({ field, message: `${label}不超过${maxLen}个字符` })
  ...
  updateCrane(
    id: string,
    input: Partial<CraneCreateInput>,
    auditInfo: { userId: string; username: string; ip: string | null; userAgent: string | null }
  ): { crane: Crane; changedFields: string[] } {
    ...
    const updateData: Partial<CraneCreateInput> = {}
    const optionalFields: Array<keyof Partial<CraneCreateInput>> = [
      'name', 'model', 'status', 'location_x', 'location_y',
      'max_load', 'max_moment', 'max_radius', 'max_height',
      'install_date', 'last_maintenance', 'manufacturer', 'serial_number',
      'production_date', 'project_name', 'construction_unit', 'registration_number',
      'min_radius', 'tip_load', 'hoist_speed', 'slewing_speed', 'trolley_speed',
      'motor_power', 'total_weight', 'jib_weight', 'counterweight',
      'free_standing_height', 'max_anchored_height', 'working_temp_min',
      'working_temp_max', 'max_wind_operational', 'max_wind_nonoperational', 'power_supply',
    ]
    ...
    const changedFields = craneRepository.getChangedFields(existing, updateData)

    if (changedFields.length === 0) {
      return { crane: existing, changedFields: [] }
    }

    const db = getDatabase()
    const tx = db.transaction(() => {
      const updated = craneRepository.update(id, updateData)
      if (!updated) throw new Error('更新失败')

      if (['max_load', 'max_moment', 'max_radius', 'max_height'].some((f) => changedFields.includes(f))) {
        craneRepository.updateSensorThresholds(id, updateData)
      }

      if (['max_load', 'max_moment', 'max_wind_operational'].some((f) => changedFields.includes(f))) {
        craneRepository.updateRuleThresholds(id, updateData)
      }

      return updated
    })

    const updatedCrane = tx()
    ...
    operationLogRepository.logCraneUpdate(
      ...
      updatedCrane as unknown as Record<string, unknown>,
      changedFields,
      auditInfo.ip,
      auditInfo.userAgent,
      true
    ...
    return { crane: updatedCrane, changedFields }
  }
}
